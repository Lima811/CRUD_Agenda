 ...
 private $oConexion=null;
		...
		$bRet = false;
			try{
				...
				$this->oConexion = new PDO("mysql:host=localhost;dbname=registros_contactos", "adminlima" , "limaglez" ,  array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES 'UTF8'"));
				$bRet = true;
			}catch(Exception $e){
				throw $e;
			}
		...
		$nAfectados = -1;
	       if ($psComando == ""){
			...
			return $nAfectados;
                ...
                //para obtener el ultimo id generado:
                public function lastInsertId() {
    if ($this->oConexion) {
        return $this->oConexion->lastInsertId(); // Usar PDO para obtener el ID
    }
    return 0; // Retorna 0 si la conexión no está activa
}
	}
 ?>
